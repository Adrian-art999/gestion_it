...
$orden_valido = $_GET['orden'] ?? 'general';
$ordenes_validos = ['general', 'recientes', 'antiguas'];
if (!in_array($orden_valido, $ordenes_validos, true)) {
    $orden_valido = 'general';
}
...
if ($filtro_sidebar === 'en_progreso') {
    $where .= " AND estado = 'En progreso' AND (fecha_limite IS NULL OR fecha_limite > '{$ahora}')";
} elseif ($filtro_sidebar === 'finalizadas') {
    $where .= " AND estado = 'Finalizada'";
...
$orderBy = 'a.id DESC';
if ($orden_valido === 'recientes') {
    $orderBy = 'a.fecha_inicio DESC';
} elseif ($orden_valido === 'antiguas') {
...
<main class="main-content">
    <div class="dashboard-header-top">
        ...
        <button onclick="abrirModal('modalReportePDF')" data-accion="reporte-pdf" class="btn-reporte-pdf">
    ...
    <div class="toolbar dashboard-toolbar">
        ...
        <select id="filtroOrden" class="filter-select dashboard-filter-select" onchange="cambiarOrden(this.value)" style="border-radius:24px !important;">
            <option value="general" <?php echo $orden_valido === 'general' ? 'selected' : ''; ?>>En general</option>
            <option value="recientes" <?php echo $orden_valido === 'recientes' ? 'selected' : ''; ?>>Más Recientes</option>
            <option value="antiguas" <?php echo $orden_valido === 'antiguas' ? 'selected' : ''; ?>>Más antiguas</option>
        </select>
    </div>
            ...
            <tbody>
                <?php if (mysqli_num_rows($resultado_actividades) > 0): ?>
                    <?php while($row = mysqli_fetch_assoc($resultado_actividades)): 
                        ...
                        $fecha_registro = !empty($row['fecha_inicio']) ? date('d-m-Y H:i', strtotime($row['fecha_inicio'])) : 'N/D';
                    ...
                    ?>
                        ...
                        $busqueda_completa = mb_strtolower(implode(' ', array_merge(
                            $responsables_full,
                            [$area, $descripcion, $estado, $duracion]
                        )));
                        ?>
    ...
    <div class="paginacion-actividades" aria-label="Paginación de actividades">
        <?php for ($p = 1; $p <= $total_paginas; $p++): ?>
            <a href="dashboard.php?estado=<?php echo urlencode($filtro_sidebar); ?>&orden=<?php echo urlencode($orden_valido); ?>&pagina=<?php echo $p; ?>" class="pagina-cuadro <?php echo ($p === $pagina_actual) ? 'activa' : ''; ?>">
                <?php echo $p; ?>
            </a>
        <?php endfor; ?>
    </div>
